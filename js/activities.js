...
G.initActivities = function () {
  ...
  // ---------- city curiosities (small interactive spots on every layer) ----------
  const SYc = G.CFG.STREET_Y, UYc = G.CFG.UNDER_Y;
  [
    // ===== STREET / ROAD LEVEL =====
    ["fish",    3050, SYc, "FISH STALL",      "A vendor flicks you a silver datafish. \"On the house, little ghost.\"", "food"],
    ["scratch", 4640, SYc, "SCRATCH POST",    "An old fiber-optic post. You sharpen your claws. It hums approvingly.", "xp"],
    ["koi",     5980, SYc, "NEON KOI POND",   "Holographic koi drift under the surface. One nudges a coin to you.", "cred"],
    ["busker",  4080, SYc, "BUSKER TIP JAR",  "You drop a paw on the synth. The busker grins and tosses you credits.", "cred"],
    ["vending", 7200, SYc, "VENDING MACHINE", "You headbutt the panel. A snack-pack thunks into the tray.", "food"],
    ["shrinecat",1380, SYc,"ALLEY CAT SHRINE","A tiny shrine of bottle caps and whiskers. You leave one of your own.", "xp"],
    ["antenna", 9600, SYc, "WARM ANTENNA",    "A humming antenna, pleasantly warm. You nap for exactly four seconds.", "xp"],
    ["dumpster",11400,SYc, "MYSTERY DUMPSTER","You dive in. Treasure! (To a cat, anyway.) Also some credits.", "cred"],
    ["holomouse",7650, SYc,"HOLO-MOUSE",      "A glitching holographic mouse darts past. You pounce — gotcha! It dissolves into kibble.", "food"],
    ["puddle",  8500, SYc, "RAIN PUDDLE",     "A perfect neon puddle. You bat at your own reflection until a coin floats up.", "cred"],
    ["ramen",   2960, SYc, "MIDNIGHT RAMEN",  "Steam curls off a bowl left to cool. You sneak a noodle. Delicious contraband.", "food"],
    ["taxi",    6300, SYc, "PARKED GRID-TAXI","You curl on the warm hood. The driver chuckles and slips you a tip.", "cred"],
    // ===== ROOFTOP LEVEL =====
    ["pigeons", 620,  1550,"ROOFTOP PIGEONS", "A row of chrome pigeons. You stalk them; they scatter in a clatter of wings. Worth it.", "xp"],
    ["rooftan", 4580, 1350,"SUNLAMP LOUNGE",  "A forgotten sunlamp still glows warm. You bask. Pure rooftop bliss.", "xp"],
    ["roofbar", 5760, 1180,"GARDEN BAR",      "A rooftop bar tended by no one. A saucer of cream sits out, just for you.", "food"],
    ["telescope",9690, 700,"OBSERVATORY",     "You nudge the telescope toward the skyline. Far off, another cat waves back.", "cred"],
    ["dish",    6900, 2300,"SATELLITE DISH",  "You climb into the warm dish and yowl. The whole plaza echoes you back.", "xp"],
    ["windsock",12330,1180,"MAST WINDSOCK",   "At the city's highest point, wind in your whiskers. You feel like a legend.", "cred"],
    // ===== UNDERGROUND LEVEL =====
    ["mush",    2150, UYc,  "GLOW MUSHROOMS",  "Bioluminescent fungi pulse in the dark. You boop one; spores rain like glitter.", "xp"],
    ["ratking", 4950, UYc,  "THE RAT COURT",   "A parliament of cyber-rats regards you. They elect to share their stash.", "cred"],
    ["drip",    6950, UYc,  "DRIP POOL",       "Water plinks from a pipe into a still pool. You drink. Cold, clean, ancient.", "food"],
    ["graff",   10300,UYc, "TUNNEL MURAL",    "A glowing mural of a thousand cats. You add a single neon pawprint.", "xp"],
    ["jukebox", 8100, UYc,  "BROKEN JUKEBOX",  "You smack it. One last song crackles out, then a forgotten coin rolls free.", "cred"],
    ["nest",    3300, UYc,  "STRAY NEST",      "A nest of warm rags and old wires. You knead it happily and find a snack.", "food"],
  ].forEach((c, i) => G.curiosities.push({
    id: "cur" + i, x: c[1], y: c[2], name: c[3], text: c[4], reward: c[5], cool: 0, ph: i * 1.3,
  }));
};
...
G.updateActivities = function (dt) {
  ...
  for (const c of G.curiosities) { c.cool = Math.max(0, c.cool - dt); if (c.cool <= 0) consider(c.x, c.y - 40, "curio", c, c.name); }
...
G.doInteract = function () {
  ...
  else if (it.type === "curio") {
    const c = it.obj;
    c.cool = 45; // re-usable after a cooldown
    G.ui.dialog ? G.ui.dialog(c.name, c.text) : G.ui.notify(c.text, "");
    if (c.reward === "food") { G.state.counts.food++; G.audio.coin(); if (G.flow) G.flow.feed(); if (G.fx) G.fx.burst(c.x, c.y - 30, "#ffb347", 10); }
    else if (c.reward === "cred") { G.earn(8, c.name); G.audio.coin(); if (G.fx) G.fx.burst(c.x, c.y - 30, "#ffe14d", 10); }
    else if (c.reward === "xp") { if (G.gainXP) G.gainXP(20); G.audio.mew(); if (G.fx) G.fx.burst(c.x, c.y - 30, "#aaff00", 10); }
    G.state.miniGames["curio-" + c.id] = true;
    G.audio.chime(); G.saveGame();
  }
