...
G.render = function (dt) {
  ...
  // camera — follow the cat in-game; hold a fixed cinematic frame on the title screen
  if (G.state.started) {
...
function drawBuildings(view, night, now) {
  ...
  for (const b of G.world.buildings) {
    ...
    // open ground-floor arcade (walkable street corridor)
    if (b.ground != null) drawGroundFloor(b, night, now);
    // water tower
    if (b.tower) {
...
// open ground-floor storefront arcade beneath a building — recessed, lit, walkable
function drawGroundFloor(b, night, now) {
  const SY = G.CFG.STREET_Y, gy = b.ground, gh = SY - gy;
  const out = !inOutage(b.x + b.w / 2);
  // recessed dark interior (covers the facade texture's solid base)
  ctx.fillStyle = "#070a12";
  ctx.fillRect(b.x, gy, b.w, gh);
  // overhang lip / soffit with under-glow
  ctx.fillStyle = "#15121f";
  ctx.fillRect(b.x, gy, b.w, 10);
  ctx.fillStyle = `rgba(0,240,255,${0.10 + 0.18 * night})`;
  ctx.fillRect(b.x, gy + 10, b.w, 2);
  // support pillars (visual only — they do not block foot traffic)
  ctx.fillStyle = "#1a1726";
  ctx.fillRect(b.x + 4, gy, 14, gh);
  ctx.fillRect(b.x + b.w - 18, gy, 14, gh);
  // evenly spaced storefront bays with lit windows + a doorway
  const seed = G.makeRng(b.seed + 7);
  const bayW = 88, pad = 26;
  const palette = ["255,179,71", "0,240,255", "255,43,214", "170,225,255"];
  for (let x = b.x + pad; x < b.x + b.w - pad - 50; x += bayW) {
    const col = palette[Math.floor(seed() * palette.length)];
    const lit = out && seed() < 0.82;
    // window pane
    ctx.fillStyle = lit ? `rgba(${col},${0.16 + 0.28 * night})` : "rgba(20,26,40,0.6)";
    ctx.fillRect(x, gy + 26, bayW - 28, gh - 46);
    // sill + frame
    ctx.strokeStyle = "rgba(255,255,255,0.08)"; ctx.lineWidth = 1;
    ctx.strokeRect(x, gy + 26, bayW - 28, gh - 46);
    // doorway
    ctx.fillStyle = lit ? `rgba(${col},${0.22 + 0.3 * night})` : "rgba(14,18,28,0.7)";
    ctx.fillRect(x + bayW - 26, gy + 40, 12, gh - 40);
    // tiny neon shop strip over the bay
    if (lit) {
      ctx.fillStyle = `rgba(${col},${0.5 + 0.4 * Math.sin(now * 3 + x)})`;
      ctx.fillRect(x, gy + 18, bayW - 28, 3);
    }
    // warm spill on the pavement
    if (lit && night > 0.2) {
      const g = ctx.createLinearGradient(0, SY - 8, 0, SY + 30);
      g.addColorStop(0, `rgba(${col},${0.10 * night})`); g.addColorStop(1, `rgba(${col},0)`);
      ctx.fillStyle = g; ctx.fillRect(x - 6, SY - 8, bayW - 8, 38);
    }
  }
}
...
function drawCuriosities(view, now) {
  for (const c of (G.curiosities || [])) {
    if (c.x < view.x - 60 || c.x > view.x + view.w + 60) continue;
    const ready = c.cool <= 0;
    const col = ready ? "#ffe14d" : "#3a4a55";
    const y = c.y - 30 + (ready ? Math.sin(now * 2 + c.ph) * 4 : 0);
    // glowing diamond beacon
    ctx.save(); ctx.translate(c.x, y); ctx.rotate(Math.PI / 4);
    ctx.fillStyle = col; ctx.shadowColor = col; ctx.shadowBlur = ready ? 12 : 0;
    ctx.globalAlpha = ready ? 0.9 : 0.5;
    ctx.fillRect(-6, -6, 12, 12);
    ctx.shadowBlur = 0; ctx.globalAlpha = 1;
    ctx.fillStyle = "#04121a"; ctx.fillRect(-3, -3, 6, 6);
    ctx.restore();
    if (ready) {
      ctx.fillStyle = "rgba(255,225,77,0.85)"; ctx.font = "11px monospace"; ctx.textAlign = "center";
      ctx.fillText(c.name, c.x, y - 16);
      ctx.textAlign = "left";
    }
  }
}
