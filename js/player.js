...
G.initPlayer = function (x, y) {
  G.player = {
    ...
    stepPhase: 0, climbTick: 0, purrT: 0, scratchT: 0,
  };
};
...
G.updatePlayer = function (dt) {
  ...
  // ================= SCRATCH (expressive activity — press C) =================
  if (k.scratchP) {
    k.scratchP = false;
    if (p.onGround && !p.zip && !p.climbing && !p.hanging && p.dashT <= 0 && p.scratchT <= 0) {
      p.scratchT = 0.95; p.scratchTick = 0; G.audio.scratch();
    }
  }
  if (p.scratchT > 0) {
    p.scratchT -= dt; p.idleT = 0;
    p.vx *= Math.pow(0.0001, dt); // hold position
    p.scratchTick = (p.scratchTick || 0) - dt;
    if (p.scratchTick <= 0) { p.scratchTick = 0.22; G.audio.scratch(); }
    if (G.fx && Math.random() < 0.6) { // claw flecks near the front paws
      const sx = p.x + p.facing * (p.w / 2 + 2);
      G.fx.add({ x: sx, y: p.y + 6 + (Math.random() * 8 - 4), vx: p.facing * 45, vy: -18 - Math.random() * 30, life: 0.4, type: "burst", color: "#dfe6f0", size: 2 });
    }
    p.vy += C.GRAVITY * dt;
    moveAndCollide(p, dt);
    if (!p.onGround) p.scratchT = 0; // fell off — stop scratching
    return;
  }
  ...
  if (p.climbing) {
    ...
    if (k.up || k.down) { p.climbTick -= dt; if (p.climbTick <= 0) { p.climbTick = 0.26; G.audio.climbTick(); } }
    if (p.y < l.y - 6) { // top exit
      p.climbing = null;
      if (l.type === "ugladder") {
        // the hole has no floor — snap firmly onto the nearest street slab beside it
        let best = null, bestD = 1e9;
        for (const s of G.world.solids) {
          if (s.type !== "street" || Math.abs(s.y - l.y) > 40) continue;
          const edge = (l.x + 13) < s.x ? s.x : s.x + s.w; // near edge of this slab
          const d = Math.abs(edge - (l.x + 13));
          if (d < bestD) { bestD = d; best = { s, onLeft: (l.x + 13) >= s.x }; }
        }
        if (best) {
          const s = best.s;
          p.x = best.onLeft ? s.x + s.w - p.w / 2 - 1 : s.x + p.w / 2 + 1;
          p.y = s.y - p.h / 2 - 1; p.vx = 0; p.vy = 0; p.onGround = true;
          p.facing = best.onLeft ? -1 : 1;
        } else { p.y = l.y - 18; p.vy = -240; }
      } else {
        p.y = l.y - 18; p.vy = -240;
      }
    }
  ...
  // ================= FOOTSTEP & PURR AUDIO =================
  if (p.onGround && p.speedTier > 0 && p.slideT <= 0) {
    p.stepPhase += Math.abs(p.vx) * dt;
    if (p.stepPhase > 26) { p.stepPhase = 0; G.audio.step(p.speedTier); }
  } else p.stepPhase = 0;
  if (p.idleT > 6) { // resting → contented purr
    p.purrT -= dt;
    if (p.purrT <= 0) { p.purrT = 2.6 + Math.random() * 1.4; G.audio.purr(); }
  } else p.purrT = 0;
};
...
G.drawPlayer = function (ctx) {
  ...
  if (p.scratchT > 0) { // reared up, front paws swiping
    const sw = Math.sin(p.tailT * 32) * 5, c = catColors();
    ctx.scale(p.facing, 1);
    ctx.rotate(-0.34);
    drawCatBody(ctx, p.legT, 0, false, p);
    ctx.strokeStyle = c.white; ctx.lineWidth = 5; ctx.lineCap = "round";
    ctx.beginPath();
    ctx.moveTo(10, -2); ctx.lineTo(25, -11 + sw);
    ctx.moveTo(8, 3); ctx.lineTo(23, -3 - sw);
    ctx.stroke();
    ctx.strokeStyle = "rgba(0,240,255,0.7)"; ctx.lineWidth = 1.4;
    ctx.beginPath(); ctx.moveTo(25, -11 + sw); ctx.lineTo(31, -13 + sw);
    ctx.moveTo(23, -3 - sw); ctx.lineTo(29, -5 - sw); ctx.stroke();
    ctx.restore(); return;
  }
