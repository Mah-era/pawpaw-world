...
G.buildWorld = function () {
  ...
  G.BUILDINGS.forEach((spec, i) => {
    ...
    if (i === 2) {
      ...
      // open ground floor: the bottom GROUND px is a walkable storefront arcade,
      // so the whole street is one continuous promenade. Slim corner pillars keep
      // the tower visually supported without blocking foot traffic.
      S(bx, roofY, bw, bh - C.GROUND, "bldg");
      b.ground = SY - C.GROUND;
    }

    // EVERY building gets exactly one climbable street → roof drainpipe so the
    // whole rooftop layer is reachable from the road (no duplicates anywhere).
    b.pipeX = bx + bw - 24;
    L(b.pipeX, roofY, SY - roofY, "pipe");
  ...
  // ----- extra climb ladders (each building already has its own street→roof pipe) -----
